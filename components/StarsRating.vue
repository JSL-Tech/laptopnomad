<template>
  <div class="rating-holder">
    <div :class="'c-rating' + interactivity + 'c-rating--' + size" :data-rating-value="ratingVal">
      <button>1</button>
      <button>2</button>
      <button>3</button>
      <button>4</button>
...
@mixin c-rating($star-size: 20px, $star-element: button, $interaction-type: representative) {
  ...
  #{$star-element} {
    ...
    height: $star-size;
    border: 0;
    text-indent: -9999px; // to hide the text
    overflow: hidden;
    outline: none;
  ...
  @each $rating-value in rating-values() {
    ...
    $rating-value-ceil: ceil($rating-value); //ceiling
...
.c-rating {
  @include c-rating(toem(20px), button);
  &--clickable {
